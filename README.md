...
> for Printed Circuit Assembly Manufacturing.* (manuscript).
...
### Board-Model Distribution (Table I in paper)
...
### Defect Category Distribution (Table II in paper)
...
## Baseline Experiments (Paper §V)

Board-level defect prediction using **only mounting-process features**
(10 handcrafted features: total production time, error count, stop time, error
frequency per 100 s, stop-time ratio, etc.). Stratified split, RF / SVM / LR.

### Combined dataset (all 4,178 boards)

| Algorithm | ROC-AUC | Average Precision | P @ Recall ≥ 0.90 |
| --------- | ------: | ----------------: | ----------------: |
| RF        |   0.675 |            0.265  |            0.230  |
| SVM       |   0.604 |            0.216  |            0.219  |
| LR        |   0.600 |            0.207  |            0.219  |

Mounting-only signal is meaningful but limited under cross-model heterogeneity.

### Top-5 individual board models by AP

| Model                          | Algorithm | AP     | P @ R≥0.90 |
| ------------------------------ | --------- | -----: | ---------: |
| SEIL-IDS_9100_MAIN_TOP-250627  | SVM       | 0.6533 |     0.3333 |
| SEIL-IDS_9100_MAIN_TOP-250627  | LR        | 0.6488 |     0.2273 |
| ROOMCON_INI-241115             | LR        | 0.5017 |     0.3939 |
| ROOMCON_INI-241115             | RF        | 0.4472 |     0.4186 |
| ROOMCON_INI-241115             | SVM       | 0.4222 |     0.3861 |

→ Isolating model-specific characteristics substantially improves predictive
performance. Reflow thermal features (not used in baseline) are expected to
further close the gap and are released for community exploration.
